import hashlib
import gmpy2

from Crypto.Cipher import AES

# 2hex = 8bit = 1byte
g = 
p = 

our_privatekey = 
their_publickey = 

# Google mandou transformar em string, mas n sei se ta correto
# se pa tem q fazer direto com os bytes do numero
common_key = gmpy2.powmod(their_publickey, our_privatekey, p)
sha256_hash = hashlib.sha256(str(common_key).encode('utf-8'))
key = sha256_hash.digest()[:16]

received_message = bytes.fromhex("f14c3e0f30afe2b3d6ca2e5ac9d8cd4eadb5716236ee27ff7895cfca62e502eaaea51a9cc5a549758fe5339de292a84b925d7dcd7250fb50f3bf9e8c886f84f4")

iv = received_message[:16]
ciphertext = received_message[16:]

print(f"{key=}")
print(f"{iv=}")
print(f"{ciphertext=}")

def pad(data, block_size=16):
    pad_length = block_size - (len(data) % block_size)
    return data + bytes([pad_length] * pad_length)

def unpad(data):
    pad_length = data[-1]
    return data[:-pad_length]

def xor_bytes(a, b):
    return bytes([x ^ y for x, y in zip(a, b)])

def simple_aes(block, key):
    # Se for usar lib, colocar aq a implementação
    return xor_bytes(block, key)

def decrypt_aes_cbc(key, iv, ciphertext):
    cipher = AES.new(key, AES.MODE_CBC, iv)
    original_message = unpad(cipher.decrypt(ciphertext))
    return original_message

# def decrypt_aes_cbc(key, iv, ciphertext):
#     plaintext = b""
#     previous_block = iv

#     for i in range(0, len(ciphertext), 16):
#         block = ciphertext[i:i + 16]
#         decrypted_block = simple_aes(block, key)
#         decrypted_block = xor_bytes(decrypted_block, previous_block)
#         plaintext += decrypted_block
#         previous_block = block

#     return unpad(plaintext, 16)

print(f'decrypted={decrypt_aes_cbc(key, iv, ciphertext).decode('utf-8')}')